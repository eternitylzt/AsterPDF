...
- **Markdown 与视频**：阅读含数学公式的 Markdown 并导出 PDF；可在 PDF 中插入视频、提取内嵌媒体并调整媒体层级。
...
- **离线数学公式**：Markdown 的行内、独立公式和 `math` 代码块可直接排版，导出的 PDF 保留矢量质量。
- **更接近 GitHub 的排版**：改善嵌套列表、列表内代码块、表格、任务列表、链接及图片比例。
- **不再等待打印机**：打开 Markdown 直接生成 PDF，不连接系统默认打印机。
